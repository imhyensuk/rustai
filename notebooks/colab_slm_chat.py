...
def gather(question: str):
    ...
    for n, meta in enumerate(res.context.sources, start=1):
        ...
        cites.append((n, meta["title"], meta["url"], meta["tokens"]))
...
while True:
    ...
    if q == "/출처":
        if last:
            for n, title, url, tk in last["cites"]:
                print(f"  [{n}] {title[:60]}\n      {url}  ({tk} 토큰)")
        else:
            print("  아직 없습니다."); 
        continue

    # 후속 질문이 대명사로만 이뤄지면 직전 질문을 붙여 검색어를 만듭니다.
    query = q if len(q) > 12 or not history else f"{history[-1]['q']} {q}"
    ...
    print(f"\n  ({dt:.1f}초 · {n}토큰 · {n/dt:.0f} tok/s)")
    print("\n  출처:")
    for num, title, url, tk in g["cites"]:
        print(f"   [{num}] {title[:58]}\n       {url}")
